    ...
    return (
        ...
        <form action="" className="newCustomer" onSubmit = {handleSubmit(Submit)}>
                <div className="Item">
                    ...
                    <input type="text" placeholder = "name of customer" required autoComplete = "off" {...register('Customer',{required:true,maxLength:8})} onChange = {e => {setSuggestionCustomer(e.target.value)} } />
                      ...
                      <ul className = "listSuggestion">
                        {customerList.filter(val => {
                            if(suggestionCustomer.length === 0){
                                return null
                            }else if(val.name.toString().toLowerCase().includes(suggestionCustomer.toString().toLowerCase())){
                                return val
                            }
                        }).map(item => {
                            return (
                ...
                <div className="Item">
                    ...
                    <input type="text" placeholder = "name of doctor" autoComplete = "off" required {...register('Doctor',{required:true,maxLength:30})} onChange = {e => {setSuggestionDoctor(e.target.value)} } />
                      {errors.Doctor && errors.Doctor.type === "maxLength" && (
                        <span role="alert"> <p style = {{color : "red"}}>Max length 30 caracteres</p></span>
                      )}
                      <ul className = "listSuggestion">
                      {dcotorsList.filter(val => {
                          if(suggestionDoctor.length === 0){
                              return null
                          }else if(val.name.toString().toLowerCase().includes(suggestionDoctor.toString().toLowerCase())){
                              return val
                          }
                      }).map(item => {
                          return (
                ...
                <div className="Item">
                    ...
                    <input type="text" placeholder = "name of hospital" {...register('Hospital',{required:true,maxLength:30})} onChange = {e => {setSuggestioHospital(e.target.value)} } />
                    {errors.Hospital && errors.Hospital.type === "maxLength" && (
                      <span role="alert"> <p style = {{color : "red"}}>Max length 30 caracteres</p></span>
                    )}
                    <ul className = "listSuggestion">
                    {hospitalsList.filter(val => {
                        if(suggestionHospital.length === 0){
                            return null
                        }else if(val.name.toString().toLowerCase().includes(suggestionHospital.toString().toLowerCase())){
                            return val
                        }
                    }).map(item => {
                        return (
                ...
                <div className="Item btm">
                    ...
                    <input type="date" placeholder = "06 XX XX XX XX" required {...register('date',{required:true,maxLength:10})} />
                ...
                <div className="Item btm">
                    ...
                    <input type="text" placeholder = "time of the meeting" required {...register('Time',{required:true,maxLength:30})} />
